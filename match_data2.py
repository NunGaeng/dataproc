...
correct_pred = 0
#to_extract = 'totalDamageDealtToChampions'
#to_extract = 'kda'
to_extract = '14m gold'

for j, match in enumerate(data):
    total_red, total_blue = 0,0
    total_kda_blue, total_kda_red = 0,0
    ...
    pred_winner = "블루" if total_kda_blue > total_kda_red else "레드"
    ...
    print(f"{to_extract} : 레드팀 - {total_kda_red}, 블루팀 - {total_kda_blue}")
    print(f"승리팀 : {winner}\n")
